...
const UserRow = React.memo(function UserRow({ item }: { item: ManagedUser }) {
...
export function UserManagementScreen() {
  ...
  const renderItem = useCallback(({ item }: { item: ManagedUser }) => (
    <UserRow item={item} />
  ), []);

  const FilterChip = useCallback(({ label, active }: { label: string | null; active: boolean }) => (
    <Pressable
  ...
  const filters = [null, 'user', 'nurse', 'family', 'admin'] as const;
