...
export const personalInfoSchema = z.object({
  age: z.coerce.number({ invalid_type_error: 'Required' }).min(13, 'Min 13 years').max(120, 'Max 120 years'),
  heightCm: z.coerce.number({ invalid_type_error: 'Required' }).min(50, 'Min 50 cm').max(250, 'Max 250 cm'),
  weightKg: z.coerce.number({ invalid_type_error: 'Required' }).min(20, 'Min 20 kg').max(300, 'Max 300 kg'),
});
...
export const lifestyleSchema = z.object({
  ...
  sleepHours: z.coerce.number({ invalid_type_error: 'Required' }).min(0).max(24),
