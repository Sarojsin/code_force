...
3. **SQLite:** Remains unchanged (Scenario 32).
...
**User Action:** Ananya logs in on Priya's device (without Priya logging out first—impossible because of the auth flow, but let's assume Priya logged out and Ananya logs in).
...
For users who share devices (e.g., a family tablet in a hospital), privacy-conscious users may demand that all traces of their data be removed upon logout.
