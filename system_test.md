Corrected Scenario 1: Priya confirms a period on the predicted date (Online)
Correction: I mistakenly said the model predicted the 14th, but the block is based on the predicted Start Date + Average Length.
...
Checkpoints: ✅ UI updates < 100ms. ✅ Avg Period Length remains stable (she confirmed the exact prediction).

✅ Corrected Scenario 2: Priya logs a period manually (Ignores Sticky Card)
Correction: The user does not enter an End Date by default. The system computes it.

Action:

Priya ignores the Sticky Card. On June 16, she opens LogPeriodScreen.

She selects Start Date: June 15 (the actual start).

She leaves the End Date empty (or the system auto-fills it based on her average).

Expected System Behavior:

The system auto-links this log to the prediction (Start: June 15, within ±5 days).

It auto-fills period_end_date = June 15 + (avg_period_length - 1) = June 19.

It updates the old prediction with actual_cycle_entry_id.

Calendar shows Dark Pink June 15–19.

Average Period Length remains 5 days (unchanged).

🆕 Scenario 2B: Priya overrides the End Date (Correcting the Average)
Action (The Edge Case you mentioned):
...
Checkpoints: ✅ Manual End Date overrides average. ✅ avg_period_length updates successfully.
...
Updated Test Scenarios (Based on your correction)
...
State Check: Today (June 15) <= Start_Date (June 15) + Avg (5) - 1 → State B (Active within window).
...
Checkpoints: ✅ Early start date is confirmed instantly. ✅ Avg Error shifts negative.
...
Checkpoints: ✅ Old periods are stored correctly. ✅ System prevents logging a Start Date without an End Date when the window has obviously passed.
